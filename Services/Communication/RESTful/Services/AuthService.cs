...
public class AuthService : IAuthService
{
    ...
    public async Task<ApiResult<string>> LoginAsync(LoginRequest request)
    {
        ...
        if (result.IsSuccess && result.Data is not null)
            return ApiResult<string>.Success(result.Data.Token,"Succes", result.StatusCode.GetValueOrDefault(HttpStatusCode.OK));

        return ApiResult<string>.Failure(result.ErrorMessage ?? "Error al iniciar sesión", "Error", result.StatusCode);
    ...
    public async Task<ApiResult<string>> SendCodeEmailAsync(SendCodeRequest request)
    {
        ...
        return ApiResult<string>.Failure(result.ErrorMessage ?? "Error al enviar el código", "Error", result.StatusCode);
    }
    ...
    public async Task<ApiResult<string>> VerifyCodeAsync(VerifyCodeRequest request)
    {
        ...
        return ApiResult<string>.Failure(result.ErrorMessage ?? "Error al verificar el código", "Error", result.StatusCode);
    }
}
